...
try
	...
	set rawJsonCriteria to "{\"type\":\"next_actions\",\"hideCompleted\":true}"
	--	set rawJsonCriteria to "{\"type\":\"next_actions\",\"hideCompleted\":true}"
	set delaySeconds to 10
	...
	tell application "OmniFocus"
		...
		try
			-- Using "open location" as it's generally more robust for URL schemes
			GetURL theURL
			log "open location command executed. The OmniJS plugin should have run."
		on error errMsgOpen number errNumOpen
			set errorMsg to "AppleScript Error during open location: " & errMsgOpen & " (Number: " & errNumOpen & ")"
			log errorMsg
			error errorMsg
		end try
